import { CalendarIcon, CompanyBuildingColorIcon, CompanyIcon, LocationIcon } from '@/assets/icons';
...
const Description = styled.pre<any>`
	display: -webkit-box;
	font-family: inherit;
	-webkit-line-clamp: ${(props) => (props.showed ? 'none' : 2)};
...
`;
const WorkCard = (props: WorkExperienceProps) => {
	const [showDesc, setShowDesc] = useState(false);
	return (
		<FlexBox justify="start" align="start" gap={20} width={props.width}>
			<SIcon>
				...
				<FlexBox justify="space-between">
					<div>
						<FlexBox gap={10} justify="start" className="mt-10">
							...
							<FlexBox>
								<SSpan>{new Date(props.startDate!)?.toLocaleDateString('en-US', dateOptions)} - </SSpan>
								<SSpan> {new Date(props.endDate!)?.toLocaleDateString('en-US', dateOptions)}</SSpan>
							</FlexBox>
						</FlexBox>
						<FlexBox gap={30} justify="start" className="mt-10">
							...
							<FlexBox gap={5} justify="start">
								...
								<SSpan>{props.location?.name}</SSpan>
							</FlexBox>
						</FlexBox>
					</div>
				</FlexBox>

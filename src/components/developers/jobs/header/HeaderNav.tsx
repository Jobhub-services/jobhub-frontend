...
import { PHeaderNav } from '@/models/component/developer/jobs.interface';
...
const SHeaderNav = styled.div<any>`
	display: flex;
...
const HeaderNav = ({ title, badge, active = false, onClick, status }: PHeaderNav) => {
	return (
		<SHeaderNav active={active} onClick={(event: React.SyntheticEvent) => onClick(status!, event)}>
			<span>{title}</span>

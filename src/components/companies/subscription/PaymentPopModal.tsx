...
`;
const PaymentPopModal = ({ open, onClose }: { open: boolean; onClose: Function }) => {
	const { billingInfo, paymentMethod, isFetchingPaymentMethod, isBillingFetching, createSubscription, subscription } = useAppSelector(
			...
			<PopModel.Body>
				<FlexBox align="start" gap={20}>
					...
					<SOrder>
						...
						<div>
							<FlexBox justify="space-between">
								<SSubType>{selectedSubs?.title}</SSubType>
								<span>
									<strong>${amount}</strong>
								</span>
							</FlexBox>
							<div className="mt-10">Upgrade to {createSubscription?.subscriptionType} plan</div>
						</div>
						<HrDivider top={15} />
						<FlexBox justify="space-between">
